...
export default function ClientDashboardPage() {
  ...
  const { loading, error, profile, reservations, notifications, documents } = useClientPortal();
  ...
  const renderReservationCard = (reservation, variant) => {
    const start = reservation.scheduled_start ? new Date(reservation.scheduled_start) : null;
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs font-semibold uppercase tracking-[0.3em]">
          <Button variant="ghost" onClick={() => handleViewDetails(reservation)}>
            View details
          </Button>
          {variant === 'upcoming' ? (
            <>
              <Button variant="ghost" onClick={() => handleAction('reschedule', reservation)}>
                Reschedule
              </Button>
              <Button variant="ghost" onClick={() => handleAction('cancel', reservation)}>
                Cancel
              </Button>
            </>
          ) : (
  ...
  return (
    <div className="space-y-8">
      <SectionTitle
        ...
        title="Dashboard"
        description="Track reservations, upload inspiration, and stay updated with the studio."
      />
      ...
      <Card>
        <div className="grid gap-6 md:grid-cols-[1fr_280px] md:items-center">
          <div className="space-y-3">
            ...
            <h1 className="text-2xl font-semibold text-gray-900">Track reservations, share inspiration, and stay informed with ease.</h1>
          </div>
            ...
            {nextReservation ? (
              <div className="space-y-2">
                ...
                <p className="text-lg font-semibold text-gray-900">
                  {formatSessionDate(nextReservation.scheduled_start)} · {formatSessionTime(nextReservation.scheduled_start)}
                </p>
                <Button variant="ghost" onClick={() => handleViewDetails(nextReservation)}>
                  View details
                </Button>
              </div>
            ) : (
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-gray-500">Upcoming reservation</p>
              <p className="text-sm text-gray-500">All details are read-only here; toggle to edit via Profile.</p>
            </div>
            <Button as={Link} to="/portal/reservations" variant="ghost">
              View all
            </Button>
          </div>
          {nextReservation ? (
            renderReservationCard(nextReservation, 'upcoming')
          ) : (
      ...
      >
        {selectedReservation ? (
          <div className="space-y-3 text-sm text-gray-700">
            <p className="font-semibold text-gray-900">{formatFriendlyDate(selectedReservation.scheduled_start)}</p>
            <p className="text-xs uppercase tracking-[0.3em] text-gray-500">
